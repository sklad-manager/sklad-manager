...
export default function WarehouseMap({ onSlotClick, selectedSlot, refreshTrigger = 0 }: WarehouseMapProps) {
    ...
    const handleSlotInteraction = (slot: SlotData, floor: number) => {
        // Если режим перемещения НЕ активен - просто выбираем ячейку
        if (!isMoveMode) {
            onSlotClick(slot.id);
            return;
        }
            ...
            <div className="absolute top-2 right-2 z-20 flex gap-2 items-center">
                ...
                <button
                    onClick={() => {
                        ...
                        setMoveSource(null); // Сбрасываем выбор при переключении режима
                    }}
                    className={`px-3 py-1 rounded text-sm font-medium transition-colors ${isMoveMode
                            ? 'bg-blue-500 text-white'
                            : 'bg-gray-200 text-gray-700 hover:bg-gray-300'
                        }`}
                    title="Режим перемещения"
                >
                    ✋ {isMoveMode ? 'Перемещение ВКЛ' : 'Перемещение'}
                </button>
